Object의 기본 toString 메서드는 PhoneNumber@adbbd처럼 단순히 ```클래스_이름@16진수로_표시한_해시코드```를 반환합니다. toString의 일반 규약에 따르면 '간결하면서 사람이 읽기 쉬운 형태의 유익한 정보'를 반환해야 합니다. 또한 toString의 규약은 "모든 하위 클래스에서 이 메서드를 재정의하라"고 합니다.
